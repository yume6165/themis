...
createLineChart(state) {
  //初期化
  var section = document.getElementById('graph_section1')
  section.innerHTML = '';
  ...
  var width = 700,
    ...
    //csvファイルの読み込み
    var files = ["./results/output_file/img_infos.csv",
                 "./results/output_file/anken_dist/0_no_context.csv",
    ...
    Promise.all(promises).then(function(values){
        ...
        //創傷のメタデータを整える
        img_infos = img_infos.split("\n");
        img_infos.pop();
        for(var i=0; i < img_infos.length; i++){
            img_infos[i] = img_infos[i].split(",");
        }

        for(var i=0; i < img_infos.length; i++){
            for(var j=0; j < img_infos[i].length; j++){
                img_infos[i][j] = img_infos[i][j].replace(/\\/g, '/');
                //console.log(img_infos[i]);
            }
        }
        //console.log(img_infos);
        ...
        for(var i=0; i < contexts[0].length; i++){
            ...
            const wound = new woundData(i, name, img_infos[i][0], "");
            wounds.push(wound);
        }
        //console.log(wounds);
...
//                  .attr("y2", function(d){return yScale(wound_pos[d.target_index][1]);})
                ...
                var pict_node_img = g.append("g")
                    ...
                    .attr("xlink:href",function(d){
                            var img = wounds[d.wound_no].image;
                            return img;
                    })//dwelling1.svg",
                    .attr("width", 75)
                    .attr("height", 60)
...
render() {
  ...
  return <svg id="graph_section1" className="graph_section" ref={node => this.node = node}>
  </svg>
}
}
